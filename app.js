...
function showStudentsList() {
    ...
    students.forEach(student => {
        ...
        listItem.classList.add(student.attendanceStatus);
...
function markAttendance(status, listItem, selectedClass) {
    ...
    if (student) {
        ...
        listItem.className = ''; // Clear previous classes
...
function saveStudentsList(className) {
    const students = Array.from(document.getElementById('studentsList').children).map(listItem => {
        return {
            ...
            attendanceStatus: listItem.classList.contains('absent') ? 'absent' :
                             listItem.classList.contains('present') ? 'present' : 'leave'
        };
    });
